...
command syntax, container images, numerical limits) are deferred and listed in
...
| Area                   | In version 1                                                                                                                                                                    | Deferred                                                                           |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| Submission types       | GitHub issues and pull requests in public or private repositories                                                                                                               | Private vulnerability reports (GitHub PVR); non-GitHub channels (HackerOne, email) |
| Review exchanges (P08) | Passive: the steward's own report hygiene and flagging of low-value automated activity in its report                                                                            | Active moderation: minimizing comments, locking threads, interaction limits        |
| Hosting                | GitHub Actions in the target repository, GitHub Pages, contributor machines, maintainer machines                                                                                | Self-hosted webhook receiver or poller                                             |
| LLM provider           | Two shipped adapters behind one interface: Copilot SDK (GitHub token) and OpenAI-compatible HTTP (project-supplied key); the policy selects provider, model, and authentication | Additional adapters; per-stage model selection; browser inference                  |
| Sandbox                | Containers launched by trusted jobs on GitHub-hosted runners and on maintainer machines; project CI as a signal                                                                 | Other isolation technologies                                                       |
| Local CLI              | Contributor preflight, maintainer-initiated screening, historical replay, repository initialization                                                                             | —                                                                                  |
| Browser code           | One static application: contributor submission assistant and maintainer dashboard                                                                                               | Browser extension; authenticated dashboard actions                                 |
...
| 2   | Browser secrets         | None. The app reads JSON published by Actions and deep-links to github.com for every write action. It performs no inference: version 1 removes browser inference, and LLM-assisted preflight runs in the CLI.                                                                                                                                                                                                                                               |
| 3   | Submission types        | Issues and pull requests.                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| 4   | Security reports        | Deferred. The policy reserves a `security.llm` setting with values `off`, `redacted`, and `full`, default `off`, for the future private-report process.                                                                                                                                                                                                                                                                                                     |
...
| `steward-maintenance.yml` | `schedule`; `workflow_dispatch` on the default branch only                                                                           | Queued starts (calling the screening reusable workflow), stale-check reconciliation, resolution sweep, timers, audits, metrics, retention, Pages, self-test, model availability probe, policy-change rescreens                                            |
...
Ignore verified echoes of this installation's report, reminder, follow-up,
...
alone does not qualify. Keep closure/input-change propagation when a steward
...
| File                  | Content                                                                                                                                                                                     |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `data/policy.json`    | Public policy subset: categories, evidence requirements, `unrequested_change`, modes/draft guidance, length/attachment caps, dismissal codes, supported versions, inference-admission rule  |
...
| Actions `schedule`, `workflow_dispatch`            | Queued starts, stale-check reconciliation, PR resolution sweep, timers, audit sampling, rollups, retention, publication, self-test                                                                                                                                                                               | SP02, SP03, SP14, SP18, SP19 |
...
| GitHub App                                         | Bot identity, fine-grained permissions, tokens that can trigger workflows, future advisory access                                                                                                                                                                                                                | SP02, SP13, SP18             |
...
| Follow-through            | Reminder and closure timers, maximum follow-ups per cycle, whether closure is enabled.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| Security                  | Reserved: `security.llm` with `off`, `redacted`, `full`; default `off`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
backlog and waits for a maintainer decision without author timers.
...
labels, reminders, or reviewer requests. Category ambiguity cannot downgrade
...
`awaiting-author`, `triage`,
`stale`) and `claim:<classification>` for issues. Names are policy-configurable.
...
| Submission content sent to an inference provider               | Public and private repositories are supported. Installation explicitly authorizes the selected provider for the repository visibility and context; unauthorized confidential context is withheld and required missing context routes to triage. Private vulnerability report intake remains deferred with `security.llm: off`. Local CLI discloses transmitted content before sending.                                                                                                  |
...
| Event loops from the App's own writes                          | Verify installation identity and recorded resource ids to ignore report/reminder/usage/follow-up, ready-for-review, label/reaction, and maintenance-issue echoes; preserve actual dependency-change propagation; deduplicate inputs.                                                                                                                                                                                                                                                    |
...
| P06     | Preflight questions a reviewer would ask; specific requests; responsiveness assessment; timers; documented reopen path                                                                                                         | SP05, SP14                                           |
| P07     | Length caps published and enforced at intake; fixed concise report format; severity statements excluded; no chatter                                                                                                            | SP01, SP05, SP06, SP13, SP16                         |
...
- Default model per shipped adapter; per-stage model selection (deferred);
  prompt and response schema design; repair strategy for malformed output.
...
- Numerical limits: budgets, timers, retention, audit sample sizes, the
  stale-check timeout.

...
- The design docs use stable identifiers: P01–P11, N01–N05, O01–O03 (problem statement); decisions 1–16, invariants 1–8, and § references (architecture); SP01–SP20 (processes). Cite them instead of restating behavior, and keep architecture §14 traceability and each process's "Addresses" field consistent when adding one.
...
- Writes made with the App token trigger workflows. Ignore this installation's own echoes (report, reminder, usage, and follow-up comment edits; ready-for-review; labels and reactions; the maintenance issue) by verifying installation identity and recorded resource ids, and still process real dependency changes: linked-issue edits, closures, reopenings, recorded maintainer actions, and edits or deletions of response-set comments.
